'''
35. Faça um programa que apresente na tela um menu com as seguintes opções: 1. Converter um ângulo em graus para radiano; 2. Calcular o seno de um ângulo, 3. Calcular o valor de π. 4. Resolver uma equação do segundo grau; 0. Sair. Depois de feita a opção, o programa deve chamar uma função que leia do usuário os parâmetros
...
(0) - Sair""")
        ...
        if escolha == 1:
            ...
            print(f"O radiano de {angulo} é: {radianos(angulo):.2f}")
            
        elif escolha == 2:
            angulo = float(input("Digite o ângulo em graus: "))
            print(f"Sen de {angulo}: {sen(angulo):.2f}")
            
        elif escolha == 3:
            print(f"O valor de π é: {pi():.2f}")
            
        elif escolha == 4:
            a = float(input("\nDigite o valor de A da equação: "))
            b = float(input("Digite o valor de B da equação: "))
            c = float(input("Digite o valor de C da equação: "))
            print(f"O valor de (x1, x2) é: {equação_segundo(a, b, c)}")
        elif escolha == 0:
            break
